<template>
	<view class="page">
		...
		<scroll-view scroll-y class="right-view" :style="{height: winHeight+'px'}">
			<text v-show="plist.length===0" style="display: block; text-align:center;margin-top: 10px;">暂无商品</text>
			
			<uni-list>
				<!-- ellipsis：省略号，显示几行文字后呈现省略号 -->
				<!-- thumb：缩略图 -->
				<!-- thumbSize： 缩略图尺寸 -->
				<!-- link： 增加点击反馈 -->
				<!-- to： 指定跳转地址 -->
				<uni-list-item v-for="(p, i) in plist" :key="p.lid" :title="p.title" :ellipsis="2" :thumb="$base+p.pic" thumbSize="lg" link :to="`/pages/details/details?pid=${p.lid}`"/>
			</uni-list>
		</scroll-view>
	</view>
</template>
...
<script>
	...
	export default {
		watch:{
			async curCategory(){   //监视某个状态变量的更改
				console.log('curCategory发生改变，该请求对应类别下的商品了', this.curCategory);
				let kw = this.clist[this.curCategory].keywords  //当前分类的商品的查询关键字
